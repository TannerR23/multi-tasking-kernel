...
main:
    ...
    #set up $cctrl field
    sw $5, pcb_cctrl($1)
    #set up pcb timeslice
    addi $2, $0, 1
    sw $2, pcb_timeslice($1)
    #set up enabled flag
    addi $2, $0, 1
    sw $2, pcb_enabledflag($1)
    #save return address to end_task
    la $2, end_task
    sw $2, pcb_ra($1)
...
schedule:
    ...
    #check if task count is 0, if is, go to idle
    lw $13, task_counter($0)
    beqz $13, idle_task

    #check if enabledflag is 0, if so, go back to schedule and get next task
    lw $13, current_task($0)
    lw $13, pcb_enabledflag($13)
    beqz $13, schedule

    #Get timeslice from pcb and set it
    lw $13, current_task($0)
...
end_task:
    #get current task and turn its enabled flag to 0
    lw $1, current_task($0)
    sw $0, pcb_enabledflag($1)
